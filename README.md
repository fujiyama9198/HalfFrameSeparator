...
```bash
python halfframe.py <入力ディレクトリ> [--outdir 出力ディレクトリ] [--threshold 値] [--dilation 値] [--erosion 値] [--crop 値] [--no-keep-exif]
```
...
```bash
python halfframe.py <input_directory> [--outdir output_directory] [--threshold value] [--dilation value] [--erosion value] [--crop value] [--no-keep-exif]
```
